...
class IOSDeviceLiveSyncService extends device_livesync_service_base_1.DeviceLiveSyncServiceBase {
    constructor(_device, data, $iOSSocketRequestExecutor, $iOSNotification, $iOSEmulatorServices, $iOSDebuggerPortService, $logger, $fs, $processService, $platformsData) {
        super($platformsData);
    ...
    setupSocketIfNeeded(projectData) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.device.isEmulator) {
                yield this.$iOSEmulatorServices.postDarwinNotification(this.$iOSNotification.getAttachRequest(projectData.projectId, this.device.deviceInfo.identifier), this.device.deviceInfo.identifier);
                const port = yield this.$iOSDebuggerPortService.getPort({ projectDir: projectData.projectDir, deviceId: this.device.deviceInfo.identifier, appId: projectData.projectId });
                this.socket = yield this.$iOSEmulatorServices.connectToPort({ port });
                if (!this.socket) {
                    return false;
                }
            }
            else {
                yield this.$iOSSocketRequestExecutor.executeAttachRequest(this.device, constants.AWAIT_NOTIFICATION_TIMEOUT_SECONDS, projectData.projectId);
                const port = yield this.$iOSDebuggerPortService.getPort({ projectDir: projectData.projectDir, deviceId: this.device.deviceInfo.identifier, appId: projectData.projectId });
                this.socket = yield this.device.connectToPort(port);
            }
    ...
    restartApplication(deviceAppData, projectName) {
        return __awaiter(this, void 0, void 0, function* () {
