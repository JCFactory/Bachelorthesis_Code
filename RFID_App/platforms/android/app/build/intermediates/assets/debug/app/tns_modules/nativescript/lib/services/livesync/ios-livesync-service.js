...
class IOSLiveSyncService extends platform_livesync_service_base_1.PlatformLiveSyncServiceBase {
    constructor($fs, $platformsData, $projectFilesManager, $injector, $devicePathProvider, $logger, $projectFilesProvider, $iOSDebuggerPortService) {
    ...
    prepareForLiveSync(device, data, liveSyncInfo) {
    ...
    _getDeviceLiveSyncService(device, data) {
        const service = this.$injector.resolve(ios_device_livesync_service_1.IOSDeviceLiveSyncService, { _device: device, data });
        return service;
    }
}
